...
import commonparser.DocumentParser;
import commonparser.DocumentParserFactory;
import fcommon.Transport;
import file.FileTransport;
import http.HTTPTransport;
import utils.DocumentIdUtils;
import utils.UrlGroup;
...
public class BasicWebCrawler {
    ...
    public void fetchAndProcess(int maxDepth, int maxDocs) {
        ...
        int documentGroup = 1; // @balaji - Initialise the document gropu to 1 for processing the first seed urls
        
        crawlData.init();
        
        // this implicitly means that you are telling the crawler not to fetch any doc ;) so avoid this condition
        if( maxBatchSize <= 0) {
        ...
        for(int depth = 0; depth < maxDepth; depth++) {
            ...
            while( maxUrlsLimitReached == false && noMoreUrlsAtThisDepth == false) {
            	...
            	// total urls = unprocessed URLS + Processed URLs
        ...
        for( UrlGroup urlGroup : urlGroups ) {
            ...
            try {
                ...
                for(String url : urlGroup.getUrls() ) {
                    try {
                    	System.out.println("url processing   :  "+url);
    ...
    private void processPages(String groupId, 
        ...
        for(String id : docIds) {
            ...
            try {
                ...
                System.out.println("doc  "+doc+" with id "+id);
